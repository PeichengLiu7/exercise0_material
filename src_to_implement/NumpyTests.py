...
class TestGen(unittest.TestCase):
    ...
    def testDuplicate(self):
        ...
        from generator import ImageGenerator
        gen = ImageGenerator(self.file_path, self.label_path, 50, [32, 3232, 3], rotation=False, mirroring=False, shuffle=False)
        b1 = gen.next()
        b2 = gen.next()
    ...
    def testEpoch(self):
        from generator import ImageGenerator
        gen = ImageGenerator(self.file_path, self.label_path, 50, [32, 32, 3], rotation=False, mirroring=False,shuffle=True)
        gen.next()
